  ...
  return (
    <div className="space-y-8">
      ...
      <Card id="always-heavy" className="scroll-mt-20">
        <h2 className="font-heading font-semibold text-h2">{isAr ? tafkheemData.always_heavy.title_ar : tafkheemData.always_heavy.title_en}</h2>
        ...
        <div className="flex flex-wrap justify-center gap-4 mt-4">
          {tafkheemData.always_heavy.letters.map((letter) => (
            <div key={letter.arabic} className="text-center">
              ...
              <p className="text-micro text-text-muted mt-1">{isAr && letter.name_ar ? letter.name_ar : letter.name_en}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-4 p-3 rounded-lg bg-gold-light/10 dark:bg-gold-dark/10 border border-gold-light/20 dark:border-gold-dark/20 text-center">
          ...
          {!isAr && <p className="text-micro text-text-muted mt-1">{tafkheemData.always_heavy.mnemonic_en}</p>}
        </div>

        <div className="mt-4">
          <h3 className="text-micro font-semibold mb-2">{t("tafkheem.levels")}</h3>
      ...
      <Card id="always-light" className="scroll-mt-20">
        <h2 className="font-heading font-semibold text-h2">{isAr ? tafkheemData.always_light.title_ar : tafkheemData.always_light.title_en}</h2>
        <p className="text-small text-text-muted mt-2">{isAr && tafkheemData.always_light.description_ar ? tafkheemData.always_light.description_ar : tafkheemData.always_light.description}</p>

        <div className="mt-3">
          <ArabicText text={tafkheemData.always_light.letters} size="md" />
        </div>

        <p className="text-micro text-text-muted mt-2 italic">{isAr && tafkheemData.always_light.note_ar ? tafkheemData.always_light.note_ar : tafkheemData.always_light.note}</p>
      </Card>

      {/* Variable Letters */}
      <Card id="variable-letters" className="scroll-mt-20">
        <h2 className="font-heading font-semibold text-h2">{isAr ? tafkheemData.variable_letters.title_ar : tafkheemData.variable_letters.title_en}</h2>
          ...
          {tafkheemData.variable_letters.letters.map((letter) => (
            <div key={letter.arabic} className="p-3 rounded-lg bg-bg-subtle dark:bg-bg-subtle-dark">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-small font-medium">{isAr && letter.name_ar ? letter.name_ar : letter.name_en}</span>
              </div>
              <p className="text-micro text-text-muted">{isAr && letter.rule_ar ? letter.rule_ar : letter.rule}</p>
