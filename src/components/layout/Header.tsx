...
export function Header() {
  ...
  return (
    <header className="md:hidden sticky top-0 z-40 bg-bg-card dark:bg-bg-card-dark border-b border-gray-200 dark:border-gray-700">
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 top-14 z-50 bg-black/50" onClick={() => setMobileMenuOpen(false)}>
          <div className="w-[260px] h-full bg-bg-card dark:bg-bg-card-dark" onClick={(e) => e.stopPropagation()}>
            <Sidebar />
          </div>
        </div>
      )}
