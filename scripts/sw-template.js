...
//
// Strategy:
// - HTML navigation: network-first with cache fallback (never serves a stale page online).
...
function isAudio(url) {
  return /\.mp3($|\?)/i.test(url.pathname) || url.hostname.includes("quranicaudio.com") || url.hostname === "verses.quran.com" || url.hostname.includes("everyayah.com") || (url.hostname.includes("alquran.cloud") && url.pathname.includes("audio"));
}
...
async function audioCacheFirst(request) {
  ...
  if (cached) return cached;
  const network = await fetch(request);
  if (network && (network.status === 200 || network.status === 206)) {
    // Range (206) responses are not cacheable; only store full 200 bodies.
