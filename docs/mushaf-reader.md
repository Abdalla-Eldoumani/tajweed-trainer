...
- A `MushafResumeBar` opt-in "Resume listening" banner under the toolbar. The player no longer auto-restores a saved position on load (that used to hijack a play on a different surah); this banner reads the persisted `playerResume` record (the same one the `/progress` `ResumeListeningCard` uses) and plays it on tap via `usePlayer.playVerse`. Shown only when a resume record exists and nothing is playing; hidden once playback starts or it is dismissed for the session.
